...
To manage the selection between the solar panels and the batteries, a power management solution was required. A diode-ORed configuration, in which power sources are ORed together via an ideal diode, selects the source with the higher voltage while also providing the additional benefits of current sharing between the two sources and an extremely low forward voltage drop (< 100mV) compared to a typical Schottky diode. The LTC4412 (Appendix \ref{fig:ltc4412dat}) was chosen to perform this power path management. The ideal diodes are implemented externally with a FZT788B P-Channel MOSFET (Appendix \ref{fig:ftzdat}). The two LTC4412s are connected in parallel to allow for load sharing between and charging of the two battery packs.
...
The LTC6800 instrumentation opamp (Appendix \ref{fig:ltc6800dat}) was chosen to perform current sensing. A sense resistor value of 0.010$\Omega$ was chosen to provide minimal power dissipation. Although very small, since the leakage current into the opamp is also extremely small (on the order of 1nA), the trace resistance, which is approximately 0.035$\Omega$, is not a concern.
...
%@TODO Jesse: Spelling correction: 'pedal#' needs to be 'petal#' for all the solar petals in the MPPT schematic
...
The two dual-cell Lithium-Ion batteries are each managed by a BQ2057CTS. The following figure shows the battery management circuitry for one of the 2-cell battery packs. %@TODO Jesse: Describe the circuit in greater detail. ALSO: Components in the screenshot do not have identifiers (they are listed as R?, D?, etc.)
...
The calculations for the component selection for the battery management circuitry is included Appendix \ref{fig:calc???}.

\textbf{Voltage Regulation}

A diode-ORed power path management circuit was used to switch the power source that feeds the 3.3V and 5V regulation circuitry. This circuit is shown below in Figure \ref{fig:dored}. %@TODO Jesse: Describe this design in greater detail
\begin{figure}[H]
	\centering
...
The ADS1015 is a 12-bit ADC that can communicate over I2C with the microcontroller.
%@TODO Jesse: Say a little more about the design of the ADS1015
...
%@TODO Jesse: There is no discussion of the design with the ATmega. Discuss the supporting circuitry, and the connections to the I2C and other peripherals.

\textbf{Temperature and Humidity}

A temperature sensor (LM75BD) and a humidity/temperature sensor(HIH6130) will be included on the board. The schematic connections of the two ICs is shown below in Figure \ref{fig:tandh}.

%@TODO Jesse: Describe the design of the circuit and interaface with MCU, etc. Reference the ranges of the devices and that they fit within our required range
